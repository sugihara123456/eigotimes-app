...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    const prompt = `
あなたは英語学習者向けの教育教材を作成する専門家です。
「${topic}」に関連する英語学習用の模擬記事を作成してください。

【重要な制約事項】
- これは英語学習のための教育教材です
- 具体的な日付、人名、企業名、統計データは使用しないでください
- 一般的に知られている普遍的な事実のみを使用してください
- 推測や憶測に基づく内容は避けてください
- 教育目的に適した、中立的で建設的な内容にしてください

【学習者レベル】
TOEICスコア${toeicScore}点レベル（${difficultyText}）
...

{
  ...
  "wordCount": "約○○語",
  "disclaimer": "⚠️ この記事は英語学習用の教育教材として作成されています。実際のニュースではありません。学習に使用する前に、信頼できるメディア（asahi.com、NHK、日経、Yahoo!ニュースなど）で事実確認を行ってください。"
}

【記事作成ガイドライン】
- 記事は200-300語程度にしてください
- 教育的価値の高い、学習に適した内容にしてください
- 事実に基づかない創作的な要素は含めないでください
- 一般常識の範囲内で、教育的な観点から有益な内容を心がけてください
`;
